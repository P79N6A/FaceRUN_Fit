package com.fly.run.view.listview;

/**
 * Created by xinzhendi-031 on 2017/12/19.
 */
...
public class DynamicListView extends ListView implements AbsListView.OnScrollListener {
    ...
    private int maxPullHeight = DisplayUtil.screenHeight / 6;
    ...
    private void initThis(Context context) {
        maxPullHeight = DisplayUtil.dp2px(80);
...
//              log("按下，记录：%d flag:%d", downY, itemFlag);
                    ...
                    if (offset > 0 && itemFlag == ITEM_FLAG_FIRST) {
                        // 下拉
                        setSelection(0);
                        if (Math.abs(offset) > maxPullHeight)
                            offset = maxPullHeight;
